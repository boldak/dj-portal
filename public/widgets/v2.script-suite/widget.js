...
var m = angular.module("app.widgets.v2.script-suite", [
// 'app.dictionary',
"app.dps",
// 'ngFileUpload',
// 'react',
...
m.controller("ScriptSuiteController", ["$scope", "APIProvider", "dialog", "app", "i18n", "$dps", "EventEmitter", "$error", "log", "pageSubscriptions", "$q", function ($scope, APIProvider, dialog, app, i18n, $dps, EventEmitter, $error, log, pageSubscriptions, $q) {
    ...
    $scope.doDeleteScript = function () {
        ...
        var index = $scope.keys($scope.widget.script).indexOf($scope.selected);
        $scope.select();
        var newSuite = {};
        var keys = $scope.keys($scope.widget.script);
    ...
    $scope.doAddScript = function (name, script) {
        $scope.collapsed = true;
        $scope.processed = false;
        $scope.successed = false;
        $scope.rejected = false;
            ...
            fields: {
                name: {
                    ...
                    validate: function validate(form) {
                        return !$scope.widget.script[form.fields.name.value];
                    }
                }
            }
        }).then(function (form) {
...
//# sourceMappingURL=../v2.script-suite/widget.js.map