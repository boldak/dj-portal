...
var m = angular.module("app.widgets.v2.script", [
// 'app.dictionary',
"app.dps",
// 'ngFileUpload',
// 'react',
...
m.controller("ScriptController", ["$scope", "$http", "$dps", "EventEmitter", "APIProvider", "pageSubscriptions", "$lookup", "$translate", "$modal", "user", "i18n", "$scroll", "clipboard", "dialog", "$error", "log", function ($scope, $http, $dps, EventEmitter, APIProvider, pageSubscriptions, $lookup, $translate, $modal, user, i18n, $scroll, clipboard, dialog, $error, log) {
    ...
    $scope.getScript = function (s) {
        var e = $scope.examples.filter(function (item) {
            return item.title == s;
        })[0];
        if (e) {
            if (e.script) {
                ...
                $http.get(e.url).then(function (resp) {
                    e.script = resp.data;
                    $scope.script = e.script;
                    $scope.settings = {
                        options: $scope.options,
    ...
    $scope.$watch("selectedExample", function (newValue, oldValue) {
    ...
    $scope.runScript = function () {
        ...
        var s = $scope.widget.editor ? $scope.getEditorScript() : $scope.script;
...
// }
//# sourceMappingURL=../v2.script/widget.js.map