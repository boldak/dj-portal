...
angular.module("app.widgets.dm-tag-list", ["app.dictionary", "app.dps"]).controller("DataManagerTagListController", ["$scope", "$http", "$dps", "EventEmitter", "APIProvider", "pageSubscriptions", "$lookup", "$translate", "user", function ($scope, $http, $dps, EventEmitter, APIProvider, pageSubscriptions, $lookup, $translate, user) {
  ...
  $scope.selectTag = function (t) {
    ...
    $scope.$viewValue = "";
    $scope.selectItem($scope.tagList.filter(function (tag) {
      return $scope.getLabel(tag) == t;
    })[0].tag);
  };
  ...
  $scope.hasTags = function (item) {
    ...
    return $scope.selectedTags.filter(function (t) {
      return t == $scope.getLabel(item);
    }).length == 1;
  };
  ...
  $scope.selectItem = function (key) {
    ...
    // let query = [tmp];
    var query = $scope.query.split("{{}}").join(key);
  ...
  $scope.refresh = function () {
    ...
    $dps.post("/api/metadata/tag/items", { property: $scope.property, status: status }).success(function (resp) {
      $scope.processed = false;
      ...
      $scope.tagList = resp;
      $scope.tags = $scope.tagList.map(function (item) {
...
//# sourceMappingURL=../dm-tag-list/widget.js.map