...
var m = angular.module("app.widgets.v2.timeline-chart-adapter", []);

m.service("TimelineChartAdapter", ["i18n", "$lookup", "$translate", function (i18n, $lookup, $translate) {
  this.applyDecoration = function (options, decoration, selector, api, scope) {
    if (angular.isDefined(decoration) && angular.isDefined(options)) {
      ...
      options.chart.onNavigate = function (d) {
        if (scope.emit) {
      ...
      options.chart.tooltipContent = function (d) {
        var timeStamp = d.type == "instant" ? i18n.timeFormat(d.originalStart, options.timeFormat.instant) : d.type == "process" ? i18n.timeFormat(d.originalStart, options.timeFormat.process) === i18n.timeFormat(d.originalEnd, options.timeFormat.process) ? i18n.timeFormat(d.originalStart, options.timeFormat.instant) + " - " + i18n.timeFormat(d.originalEnd, options.timeFormat.instant) : i18n.timeFormat(d.originalStart, options.timeFormat.process) + " - " + i18n.timeFormat(d.originalEnd, options.timeFormat.process) : i18n.timeFormat(d.originalStart, options.timeFormat.flow) === i18n.timeFormat(d.originalEnd, options.timeFormat.flow) ? i18n.timeFormat(d.originalStart, options.timeFormat.process) + " - " + i18n.timeFormat(d.originalEnd, options.timeFormat.process) : i18n.timeFormat(d.originalStart, options.timeFormat.flow) + " - " + i18n.timeFormat(d.originalEnd, options.timeFormat.flow);

        var headline = scope.translations.translate(scope.dictionary.lookup(angular.copy(d.context)).headline);

        return "<h5" + "   style= \"font-size: 12px;" + "   font-weight: bold;" + "   margin: 0px;\">" + timeStamp + "</h5>" + "<h4" + "    style=\"font-size: 12px;" + "    margin: 0px;" + "    text-align:justify;" + "    font-stretch: ultra-condensed;" + "    line-height:1;\"" + ">" + (headline ? headline : "") + "</h4>";
      };
...
//# sourceMappingURL=../v2.nvd3-timeline/adapter.js.map