...
var m = angular.module("app.widgets.v2.geochart-adapter", []);

m.service("GeochartAdapter", ["i18n", function (i18n) {

  this.onSelectSerie = function (selection, settings, api) {
    // let data = d3.select(api.getSVG()).data();
    settings.data[0].series.forEach(function (s) {
  ...
  this.onSelectObject = function (selection, settings, api) {
    // console.log("selectObjects",selection)
    var selectedObjects = selection.filter(function (s) {
      return !s.disabled;
    });
    // console.log("features", settings.data[0].features)
    settings.data[0].features.forEach(function (o) {
      if (selectedObjects.filter(function (so) {
        return o.properties.name.en == so.key || o.properties.name.uk == so.key || o.properties.name.ru == so.key || o.properties.geocode == so.key || o.properties.name.en.indexOf(so.key) >= 0 || o.properties.name.uk.indexOf(so.key) >= 0 || o.properties.name.ru.indexOf(so.key) >= 0 || so.key.indexOf(o.properties.name.en) >= 0 || so.key.indexOf(o.properties.name.uk) >= 0 || so.key.indexOf(o.properties.name.ru) >= 0 || o.properties.name.en == so.id || o.properties.name.uk == so.id || o.properties.name.ru == so.id || o.properties.geocode == so.id || o.properties.name.en.indexOf(so.id) >= 0 || o.properties.name.uk.indexOf(so.id) >= 0 || o.properties.name.ru.indexOf(so.id) >= 0 || so.id.indexOf(o.properties.name.en) >= 0 || so.id.indexOf(o.properties.name.uk) >= 0 || so.id.indexOf(o.properties.name.ru) >= 0;
      }).length > 0) {
        o.disabled = false;
      } else {
        o.disabled = true;
...
//# sourceMappingURL=../v2.nvd3-geochart/adapter.js.map