...
angular.module("app.widgets.dm-word-cloud", ["app.dictionary", "app.dps"]).service("d3", function () {
    return d3;
}).controller("WordCloudController", ["$scope", "$http", "$dps", "EventEmitter", "APIProvider", "$lookup", "$translate", "$q", "d3", "pageSubscriptions", function ($scope, $http, $dps, EventEmitter, APIProvider, $lookup, $translate, $q, d3, pageSubscriptions) {
    ...
    function updateChart(containerID, ontology) {
        ...
        var values = ontology.tags.map(function (l) {
            return l.value;
        });
        var lvalues = ontology.links.map(function (l) {
            return l.value;
        });

        var scale = d3.scale.linear().domain([d3.min(values), d3.max(values)]).range([10, 32]);
        ...
        var linkStrokeScale = d3.scale.linear().domain([d3.min(lvalues), d3.max(lvalues)]).range([2, 8]);

        var words = ontology.tags.map(function (item, i) {
            return {
                text: item.tag,
        ...
        var links = ontology.links.map(function (item) {
            return {
        ...
        d3.layout.cloud().size([width, width]).padding(7).words(words).rotate(0) //function() { return ~~(Math.random()*2) * 90;}) // 0 or 90deg
        .fontSize(function (d) {
            return d.size;
        ...
        function drawCloud(words) {
            ...
            var labels = d3.select("#" + containerID + "-cloud").append("svg").attr("width", width).attr("height", width).append("g").selectAll("text").data(words).enter().append("text").style("font-size", function (d) {
                return d.size + "px";
            })
            ...
            g.attr("transform", function (d) {
                ...
                return "translate(" + (width / 2 - (rect.x + rect.width / 2)) + "," + -rect.y + ")";
            });
            ...
            function mouseover(cd) {
                ...
                d3.select(this).transition().ease("cubic-in").style("fill", "#CF2A0E").duration(500);

                labels.filter(function (d) {
                    return connectedWords.filter(function (w) {
                        return w.index == d.index;
                    }).length > 0;
                }).transition().ease("cubic-in").style("fill", "rgb(34, 121, 179)").duration(500);

                labels.filter(function (d) {
                    return connectedWords.filter(function (w) {
                        return w.index == d.index;
                    }).length == 0 && d.index != cd.index;
                }).transition().ease("cubic-in").style("fill-opacity", 0).duration(500);
            }

            function mouseout(d) {
                labels.transition().ease("cubic-in").style("fill", "#999999").style("fill-opacity", 1).duration(100);
            }
            ...
            function click(d) {
                ...
                // let query = [tmp];
                var query = d.query.split("{{}}").join(d.key);
    ...
    function translate() {
        ...
        $scope.nodes = [];
        $scope.links = [];
        $scope.ontology.links.forEach(function (item) {
            $scope.links.push({
                source: item.source,
                target: item.target,
                value: item.value
        ...
        $scope.ontology.tags.forEach(function (item) {
            var obj = $lookup(item.tag);
            $scope.nodes.push({
        ...
        $scope.nodes.forEach(function (item) {
            translatePromises.push($translate(item.tag).then(function (translation) {
    ...
    new APIProvider($scope).config(function () {
        ...
        console.log("Tag Cloud widget is destroyed");
    });
}]);
//# sourceMappingURL=../dm-word-cloud/widget.js.map