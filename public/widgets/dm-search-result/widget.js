...
var m = angular.module("app.widgets.dm-search-result", ["app.dictionary", "ngFileUpload", "react", "custom-react-directives"]);
m.controller("DataManagerSearchResultController", ["$scope", "$http", "EventEmitter", "APIProvider", "pageSubscriptions", "$lookup", "$translate", "$modal", "user", function ($scope, $http, EventEmitter, APIProvider, pageSubscriptions, $lookup, $translate, $modal, user) {
  ...
  var searchDatasets = function searchDatasets(query) {
    if (query) {
      ...
      $http.post("./api/metadata/items", { query: query, status: status }).success(function (resp) {
        $scope.result = resp;
  ...
  $scope.download = function (item) {
    item.download = true;
    $http.get("./api/dataset/download/" + item.dataset.id).success(function () {
      item.download = false;
    });
  };
  ...
  $scope.selectSource = function (key) {
    ...
    var query = [{ "dataset.source": [{ equals: key }] }];
    searchDatasets(query);
  };

  $scope.selectTopic = function (key) {
    eventEmitter.emit("setLookupKey", key);
    var query = [{ "dataset.topics": [{ includes: key }] }];
    searchDatasets(query);
  };
  ...
  $scope.openQueryDialog = function (item) {
    $modal.open({
      ...
      resolve: {
        item: (function (_item) {
          var _itemWrapper = function item() {
            return _item.apply(this, arguments);
          };

          _itemWrapper.toString = function () {
        ...
        prepareTopics: (function (_prepareTopics) {
          var _prepareTopicsWrapper = function prepareTopics() {
            return _prepareTopics.apply(this, arguments);
          };

          _prepareTopicsWrapper.toString = function () {
            return _prepareTopics.toString();
          };

          return _prepareTopicsWrapper;
        })(function () {
          return prepareTopics;
        }),
        table: function table() {
          return $scope.table;
        },
        formatDate: (function (_formatDate) {
          var _formatDateWrapper = function formatDate() {
            return _formatDate.apply(this, arguments);
          };

          _formatDateWrapper.toString = function () {
            return _formatDate.toString();
          };

          return _formatDateWrapper;
        })(function () {
          return formatDate;
        })
      }
      ...
      if (item.tableID) {
        $http.get("./api/table/delete/" + item.tableID).success(function () {
...
m.controller("ManageDialogController", ["$scope", "$modalInstance", "$http", "$upload", "$timeout", "item", "prepareTopics", "formatDate", "$lookup", "$translate", "user", "confirm", function ($scope, $modalInstance, $http, $upload, $timeout, item, prepareTopics, formatDate, $lookup, $translate, user, confirm) {
  ...
  $scope.upload = function (file) {
    ...
    $upload.upload({
      ...
      file: file }).then(function (response) {
      $scope.upload_process = false;
      $lookup.reload();
  ...
  $scope.upToHEAD = function (c) {
    $scope.commits = undefined;
    $http.get("./api/commit/head/" + c.metadata.dataset.commit.id).success(function (resp) {
  ...
  $scope.setCommitStatus = function (commitID, status) {
    $http.get("./api/commit/" + status + "/" + commitID).success(function (resp) {
      $scope.item = resp.metadata;
      $scope.getCommitList();
    });
  };
  ...
  $scope.deleteCommit = function (c) {
    $scope.commits = undefined;
    $http.get("./api/commit/delete/" + c.metadata.dataset.commit.id).success(function () {
      $scope.getCommitList();
    });
  };

  $scope.deleteDataset = function (c) {
    confirm("You will remove the dataset " + c.metadata.dataset.id + ". You can download it before removing. Are you sure?").then(function () {
  ...
  $scope.getAllDatasets = function () {
    $http.post("./api/metadata/items", { status: "private" }).success(function (resp) {
      $scope.datasets = resp;
    });
  };
  ...
  $scope.getCommitList = function () {
    if ($scope.item) {
      $http.get("./api/dataset/commits/" + $scope.item.dataset.id).success(function (data) {
        $scope.commits = data;
      });
    } else {
...
m.controller("QueryDialogController", ["$scope", "$modalInstance", "$http", "item", "prepareTopics", "table", "formatDate", "$lookup", "$translate", "$q", function ($scope, $modalInstance, $http, item, prepareTopics, table, formatDate, $lookup, $translate, $q) {
  ...
  var genSelectionString = function genSelectionString(dim) {
    var buf = [];
    // let s = "";
    dim.selectionString = "";
  ...
  $scope.tryGetTable = function () {
    ...
    if ($scope.requestComplete) {
      ...
      if ($scope.table) {
        $http.get("./api/table/delete/" + $scope.table.id).success(function () {
          $scope.table = undefined;
          item.tableID = undefined;

          $http.post("./api/dataset/query", $scope.request, { timeout: $scope.canceler.promise }).success(function (resp) {
            $scope.table = resp;
  ...
  $scope.makeRequest = function (item) {
    ...
    for (var i in item.dimension) {
      ...
      var collection = getSelectedItems(d);
  ...
  var getSelectedItems = function getSelectedItems(d) {
    var buf = [];
...
//# sourceMappingURL=../dm-search-result/widget.js.map