...
var m = angular.module("app.widgets.v2.steps.stacked-area-chart-decoration", ["app.widgets.palettes", "app.widgets.v2.nvd3-widget", "app.widgets.data-util.dps", "app.widgets.v2.stacked-area-chart-adapter"]);

m.factory("StackedAreaChartDecoration", ["Palettes", "$http", "parentHolder", "Requestor", "StackedAreaAdapter", function (Palettes, $http, parentHolder, Requestor, StackedAreaAdapter) {
	return {
		...
		setColor: function setColor(palette) {
		...
		activate: function activate(wizard) {
			...
			new Requestor().push("getOptions", function (requestor) {
				$http.get("./widgets/nvd3-stacked-area/options.json").success(function (data) {
					thos.options = data;
				...
				$http.post("./api/data/process/", thos.request).success(function (data) {
					thos.data = data.data;
...
//# sourceMappingURL=../v2.steps/stacked-area-chart-decoration.js.map