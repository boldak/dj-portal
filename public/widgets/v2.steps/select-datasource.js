...
m.factory("SelectDatasource", ["$upload", "$http", function ($upload, $http) {
  return {
    ...
    updateDatasourceList: function updateDatasourceList() {
      var thos = this;
    ...
    upload: function upload(file) {
      ...
      file.upload.then(function (response) {
        ...
        if (response.status > 0) thos.scope.errorMsg = response.status + ": " + response.data;
      });
    ...
    select: function select(datasource) {
      if (!this.dataSources) {
        return;
      }var thos = this;
...
//# sourceMappingURL=../v2.steps/select-datasource.js.map