...
var m = angular.module("app.widgets.v2.steps.scatter-chart-decoration", ["app.widgets.v2.nvd3-widget", "app.widgets.v2.scatter-chart-adapter", "wizard-directives", "app.dps", "ng.ace"]);

m.factory("ScatterChartDecoration", ["$http", "$dps", "$q", "parentHolder", "NVD3ScatterAdapter", "pageWidgets", "i18n", "dialog", "$error", "dpsEditor", function ($http, $dps, $q, parentHolder, NVD3ScatterAdapter, pageWidgets, i18n, dialog, $error, dpsEditor) {
	...
	return {
		...
		html: "./widgets/v2.nvd3-scatter/scatter-chart-decoration.html",
		...
		loadSeries: function loadSeries() {
			...
			if (this.conf.dataID) {
				return $dps.post("/api/data/script", {
					data: "source(table:'" + this.conf.dataID + "');" + "scatter(x:" + this.conf.axisX + "," + "index:" + JSON.stringify(this.conf.index) + "," + "category:" + this.conf.category + ");" + "save()",
					locale: i18n.locale()
				});
			}if (this.conf.script) {
...
//# sourceMappingURL=../v2.nvd3-scatter/scatter-chart-decoration.js.map