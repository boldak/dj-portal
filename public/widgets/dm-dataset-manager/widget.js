...
.controller("DatasetsManagerController", ["$scope", "$http", "$upload", "$timeout", "$dps", "$lookup", "EventEmitter", "APIProvider", "pageSubscriptions", "$translate", "user", "confirm", "alert", "dialog", "log", "progress", function ($scope, $http, $upload, $timeout, $dps, $lookup, EventEmitter, APIProvider, pageSubscriptions, $translate, user, confirm, alert, dialog, log, progress) {
    ...
    $scope.uploadCommit = function () {
        dialog({
            ...
            $http.post($dps.getUrl() + "/api/dataset/update", fd, {
                ...
                if (response.log.filter(function (item) {
                    return item.level == "error";
                }).length > 0) title = { level: "error", text: "Cannot create Dataset" };
                if (response.log.filter(function (item) {
                    return item.level == "warning";
                }).length > 0) title = { level: "warning", text: "Dataset Created with Warnings" };
    ...
    $scope.uploadfromFTP = function () {
        dialog({
            ...
            $http.post($dps.getUrl() + "/api/dataset/ch/update", fd, {
                ...
                if (response.log.filter(function (item) {
                    return item.level == "error";
                }).length > 0) title = { level: "error", text: "Cannot create Dataset Group" };
                if (response.log.filter(function (item) {
                    return item.level == "warning";
                }).length > 0) title = { level: "warning", text: "Dataset Group Created with Warnings" };
    ...
    $scope.getDatasetUUID = function () {
        ...
        $dps.get("/api/dataset/id/create").then(function (response) {
            p.close();
    ...
    $scope.exportDictionary = function () {
        ...
        $dps.get("/api/export/dictionary").then(function (response) {
            p.close();
    ...
    $scope.exportDatasets = function () {
        ...
        $dps.get("/api/export/datasets/metadata").then(function (response) {
            p.close();
            dialog({
                ...
                fields: {
                    list: {
                        title: "Dataset list",
                        type: "multiselect",
                        options: response.data.data.list.map(function (item) {
                            return {
                                value: item.id,
                ...
                $dps.get("/api/export/datasets/" + form.fields.list.value.join("+")).then(function (response) {
                    p.close();
                    $dps.downloadJSON(response.data, response.data.data.file);
    ...
    $scope.importDatasets = function () {
        dialog({
            ...
            $http.post($dps.getUrl() + "/api/import/datasets", fd, {
                ...
                if (response.log.filter(function (item) {
                    return item.level == "error";
                }).length > 0) title = { level: "error", text: "Cannot import datasets" };
                if (response.log.filter(function (item) {
                    return item.level == "warning";
                }).length > 0) title = { level: "warning", text: "Datasets imported with Warnings" };
    ...
    $scope.importDictionary = function () {
        dialog({
            ...
            $http.post($dps.getUrl() + "/api/import/dictionary", fd, {
                ...
                if (response.log.filter(function (item) {
                    return item.level == "error";
                }).length > 0) title = { level: "error", text: "Cannot import dictionary" };
                if (response.log.filter(function (item) {
                    return item.level == "warning";
                }).length > 0) title = { level: "warning", text: "Dictionary imported with Warnings" };
    ...
    $scope.uploadDictionary = function () {
        dialog({
            ...
            $http.post($dps.getUrl() + "/api/dictionary/update", fd, {
                ...
                if (response.log.filter(function (item) {
                    return item.level == "error";
                }).length > 0) title = { level: "error", text: "Cannot update dictionary" };
                if (response.log.filter(function (item) {
                    return item.level == "warning";
                }).length > 0) title = { level: "warning", text: "Dictionary updated with Warnings" };
    ...
    $scope.upToHEAD = function (c) {
        ...
        $dps.get("/api/commit/head/" + c.metadata.dataset.commit.id).success(function (resp) {
    ...
    $scope.setCommitStatus = function (commitID, status) {
        ...
        $dps.get("/api/commit/" + status + "/" + commitID).success(function (resp) {
            $scope.item = resp.metadata;
            $scope.getCommitList();
            eventEmitter.emit("refresh");
        });
    ...
    $scope.deleteCommit = function (c) {
        ...
        $dps.get("/api/commit/delete/" + c.metadata.dataset.commit.id).success(function () {
            $scope.getCommitList();
            eventEmitter.emit("refresh");
        });
    };
    ...
    $scope.getAllDatasets = function () {
        ...
        $dps.post("/api/metadata/items", { status: "private" }).success(function (resp) {
            $scope.datasets = resp;
        });
    };
    ...
    $scope.getCommitList = function () {
        if ($scope.item) {
            ...
            $dps.get("/api/dataset/commits/" + $scope.item.dataset.id).success(function (data) {
                $scope.commits = data;
            });
        } else {
...
// ;
//# sourceMappingURL=../dm-dataset-manager/widget.js.map