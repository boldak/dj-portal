...
var m = angular.module("app.widgets.v2.dm-ds-history", ["app.dictionary", "app.dps", "ngFileUpload", "react", "custom-react-directives"]);
m.controller("DataManagerDSHistoryController", ["$scope", "$http", "$dps", "EventEmitter", "APIProvider", "pageSubscriptions", "$lookup", "$translate", "$modal", "user", "i18n", function ($scope, $http, $dps, EventEmitter, APIProvider, pageSubscriptions, $lookup, $translate, $modal, user, i18n) {
  ...
  new APIProvider($scope).config(function () {
    ...
    if ($scope.ds) {
      $dps.get("/api/dataset/commits/" + $scope.ds.dataset.id).success(function (data) {
        $scope.commits = data;
      });
    }
  }).translate(function () {}).removal(function () {
...
//# sourceMappingURL=../v2.dm-ds-history/widget.js.map