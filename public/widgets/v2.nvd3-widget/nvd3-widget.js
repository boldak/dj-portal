...
define(["angular", "angular-oclazyload", "angular-nvd3", "canvg", "file-saver"], function (angular) {

  var m = angular.module("app.widgets.v2.nvd3-widget", ["oc.lazyLoad", "nvd3", "app.dps"]);

  m.factory("Selector", [function () {
    var Selector = function Selector(serieadapter, data, emitter) {
      var _this = this;
      ...
      this.object = function (objectKey) {
        return _this.objects.filter(function (o) {
          return o.key == objectKey;
        })[0];
      };

      this.selectSerie = function (serieKey) {

        if (angular.isArray(serieKey)) {
          var thos = _this;
          serieKey.forEach(function (v, index) {
            thos.series[index].disabled = v;
          });
          emitter.emit("selectSerie", _this.series);
          return;
        }

        if (_this.seriesRadioMode) {
          _this.series.forEach(function (s) {
            if (s.key == serieKey) {
              s.disabled = false;
            } else {
              s.disabled = true;
        ...
        var selectedSerie = _this.series.filter(function (s) {
          return s.key === serieKey;
        })[0];
      ...
      this.selectOneObject = function (objectKey) {
        ...
        emitter.emit("selectObject", _this.objects);
      };
  ...
  m.factory("NVD3WidgetV2", ["$http", "$dps", "$q", "$ocLazyLoad", "APIProvider", "APIUser", "i18n", "$lookup", "parentHolder", "dialog", "Selector", "EventEmitter", "pageSubscriptions", "$error", function ($http, $dps, $q, $ocLazyLoad, APIProvider, APIUser, i18n, $lookup, parentHolder, dialog, Selector, EventEmitter, pageSubscriptions, $error) {

    $ocLazyLoad.load({
      files: ["/components/nvd3/nv.d3.css", "/widgets/v2.nvd3-widget/nvd3-widget.css"]
    });

    var NVD3WidgetV2 = function NVD3WidgetV2($scope, params) {
      ...
      $scope.getContainerWidth = function () {
        var n = d3.select($scope.container.getElement()[0]).select("div .widget");
        var w = n.style("width");
        w = w.split("px")[0];
        var pad = n.style("padding");
        pad = pad.split(" ").map(function (item) {
          return item.split("px")[0];
        });
        pad = pad.length == 0 ? [0, 0, 0, 0] : pad.length == 1 ? [pad[0], pad[0], pad[0], pad[0]] : pad.length == 2 ? [pad[0], pad[1], pad[0], pad[1]] : pad.length == 3 ? [pad[0], pad[1], pad[2], pad[1]] : pad;
      ...
      $scope.expandOptions = function (options) {
        // console.log("Expand Options")
        if ($scope.widget.decoration) {
          $scope.decorationAdapter.applyDecoration(options, $scope.widget.decoration, $scope.selector, $scope.api, $scope);
        } else {
        ...
        if ($scope.widget.emitters && $scope.widget.emitters.split(",").length > 0) {
          if (options.chart.legend) options.chart.legend.slaveChart = true;
          options.chart.showControls = false;
        } else {
          if (options.chart.legend) options.chart.legend.slaveChart = false;
        }
      ...
      $scope.loadOptions = function () {
        ...
        return $q(function (resolve, reject) {
          if ($scope.options) {
            ...
            $http.get(params.optionsURL).then(function (resp) {
      ...
      $scope.updateChart = function () {
        ...
        $q.all([loadOptions().then(function (options) {
          ...
          $scope.data = params.serieAdapter && params.serieAdapter.getSeries ? params.serieAdapter.getSeries(resp.data.value) : resp.data.value;
      ...
      $scope.exportImage = function () {
        dialog({
          ...
          var filename = form.fields.fileName.value;
          filename = filename.split(".");
          filename = filename.length == 1 ? filename[0] == "" ? $scope.widget.instanceName + ".png" : filename[0] + ".png" : filename[filename.length - 1] == "png" ? filename.join(".") : filename.join(".") + ".png";
      ...
      $scope.setData = function (value) {
        ...
        $scope.data = params.serieAdapter && params.serieAdapter.getSeries ? params.serieAdapter.getSeries(value) : value;
        ...
        $scope.loadOptions().then(function (options) {
          $scope.options = options;
          $scope.settings = {
            options: angular.copy($scope.expandOptions($scope.options)),
            data: angular.copy($scope.data)
      ...
      $scope.APIProvider.config(function () {
      ...
      }).provide("selectSerie", function (e, selection) {
        if ($scope.decorationAdapter.onSelectSerie) {
          var s = $scope.decorationAdapter.onSelectSerie(selection, {
            options: angular.copy($scope.expandOptions($scope.options)),
      ...
      }).provide("selectObject", function (e, selection) {
        if ($scope.decorationAdapter.onSelectObject) {
          var s = $scope.decorationAdapter.onSelectObject(selection, {
            options: angular.copy($scope.expandOptions($scope.options)),
            data: angular.copy($scope.settings.data)
          }, $scope.api);
        ...
        if (params.acceptData(context) && (!context.widget || context.widget.indexOf($scope.widget.instanceName) >= 0)) {
          // console.log("ACCEPT")
          $scope.dataset = context.dataset;
          $scope.configured = true;
          $scope.setData(context.data);
...
//# sourceMappingURL=../v2.nvd3-widget/nvd3-widget.js.map