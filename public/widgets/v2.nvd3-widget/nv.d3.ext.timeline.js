...
function generatePath(points) {
	return d3.svg.area().interpolate("linear-closed").x(function (p, i) {
		return p.x;
	}).y0(function (p, i) {
		return p.y;
	}).y1(function (p, i) {
...
var layout = function layout() {
	...
	this._childAccessor = function (d) {
		return d.childs;
	};
	this._bands = [];
	this.line = [];
...
layout.prototype = {
	...
	childAccessor: function childAccessor(_) {
		if (!_) {
	...
	data: function data(_) {
		...
		if (!this._extent) {
			...
			(function () {
				...
				_this._data.forEach(function (serie) {
	...
	fit: function fit() {
		...
		this._data.forEach(function (serie, i) {
			thos._layout.size([thos._width, thos._height / thos._data.length]).extent(thos._extent).childAccessor(thos._childAccessor);

			var tmp = thos._layout(serie.values);
			var swimline = { y: [], dy: [] };
			tmp.forEach(function (d) {

				if (swimline.y.indexOf(d.y) < 0) {
					swimline.y.push(d.y);
				}
				if (swimline.dy.indexOf(d.dy) < 0) {
					swimline.dy.push(d.dy);
				}
			});
			swimline.dy = d3.min(swimline.dy, function (t) {
				return t;
			});
		...
		var minWidth = d3.min(swimlines, function (d) {
			return d.dy;
		});
		var sumWidth = 0;
		...
		this._bands.forEach(function (band, i) {
			band.forEach(function (item, j) {
				...
				var foundedLane = thos.lanes.filter(function (t) {
					return t.serie == lane.serie && t.lane == lane.lane;
				});
		...
		thos.line.sort(function (a, b) {
			if (date.subtract(new Date(a.originalStart), new Date(b.originalStart)).toMilliseconds() != 0) return date.subtract(new Date(a.originalStart), new Date(b.originalStart)).toMilliseconds();
			if (a.band - b.band != 0) return a.band - b.band;
...
var foTooltip = function foTooltip(w) {
	this._stopped = true;
...
foTooltip.prototype = {
	...
	dateFormat: function dateFormat(_) {
		if (!_) {
	...
	show: function show(d) {
		...
		this.contentWrapper = this._wrapper.append("foreignObject").attr(wr);

		var nav = "<center>" + "&nbsp;" + "<button class=\"navigator first\" style=\"opacity:0;font-size: 10px;margin: 2px;padding: 2px 5px;color: #666666;background: #eaeaea;border: 1px solid #666666;width:15%;" + (thos._stopped ? "" : "visibility:hidden;") + "\"> &lt;&lt; </button>" + "<button class=\"navigator prev\" style=\"opacity:0;font-size: 10px;margin: 2px;padding: 2px 5px;color: #666666;background: #eaeaea;border: 1px solid #666666;width:15%;" + (thos._stopped ? "" : "visibility:hidden;") + "\"> &lt; </button>" + "<button class=\"navigator play\" style=\"opacity:0;font-size: 10px;margin: 2px;padding: 2px 7px;color: #666666;background: #eaeaea;border: 1px solid #666666;text-align:center;\">" + (thos._stopped ? "Play" : "Stop") + "</button>" + "<button class=\"navigator next\" style=\"opacity:0;font-size: 10px;margin: 2px;padding: 2px 5px;color: #666666;background: #eaeaea;border: 1px solid #666666;width:15%;" + (thos._stopped ? "" : "visibility:hidden;") + "\"> &gt; </button>" + "<button class=\"navigator last\" style=\"opacity:0;font-size: 10px;margin: 2px;padding: 2px 5px;color: #666666;background: #eaeaea;border: 1px solid #666666;width:15%;" + (thos._stopped ? "" : "visibility:hidden;") + "\"> &gt;&gt; </button>" + "</center>";

		var c = this.contentWrapper.append("xhtml:div").append("div").attr({
			"class": "tooltip-container"
		}).html(this._content(d));

		c.append("div").html(nav);
		c.select("button.navigator.prev").on("click", function () {
			thos._chart.prev();
		});
		c.select("button.navigator.next").on("click", function () {
			thos._chart.next();
		});
		c.select("button.navigator.first").on("click", function () {
			thos._chart.first();
		});
		c.select("button.navigator.last").on("click", function () {
			thos._chart.last();
		});
		c.select("button.navigator.play").on("click", function () {
			var onPlayNext = (function (_onPlayNext) {
				var _onPlayNextWrapper = function onPlayNext(_x) {
					return _onPlayNext.apply(this, arguments);
				};

				_onPlayNextWrapper.toString = function () {
					return _onPlayNext.toString();
				};

				return _onPlayNextWrapper;
			})(function (tooltip) {
				thos._chart.next();
		...
		c.selectAll("button.navigator").transition().duration(500).style("opacity", 1);
		...
		d3.timeout(function () {
			...
			if (thos._height > 0) {
				...
				thos._wrapper.insert("path", ":first-child").attr("d", function (d, i) {
					return generatePath([fo.lt, fo.rt, fo.sa, fo.ma, fo.ea, fo.rb, fo.lb, fo.lt]);
				}).attr("transform", "translate(" + thos._x + "," + thos._y + ")").style("stroke", d3.rgb(thos._color).darker(1)) //this._color)
				.style("stroke-width", 0.6).style("fill", thos._bgColor) //d3.rgb(this._color).brighter(5.4))
				.style("fill-opacity", 1).style("filter", "url(" + document.location.href + "#drop-shadow)");
				thos._wrapper.transition().duration(500).ease("cubic-in").attr("opacity", 1);
			}
		}, 50);
	},
...
nv.models.timelineChart = function () {
	...
	var margin = { top: 20, right: 20, bottom: 20, left: 20 },
	    ...
	    legend = nv.models.legend().key(function (d) {
		return d.category;
	    ...
	    showNavigator = true,
	    onNavigate = function onNavigate(d) {
	...
	var insertSerieIndex = (function (_insertSerieIndex) {
		var _insertSerieIndexWrapper = function insertSerieIndex(_x, _x2) {
			return _insertSerieIndex.apply(this, arguments);
		};

		_insertSerieIndexWrapper.toString = function () {
			return _insertSerieIndex.toString();
		};

		return _insertSerieIndexWrapper;
	})(function (values, i) {
	...
	var getDomain = function getDomain(_data) {
	...
	function chart(selection) {
		...
		selection.each(function (_d) {
			...
			var data = _d.filter(function (item) {
				return item.disabled == false || item.disabled == undefined;
			});
			...
			var xScale = d3.time.scale().domain(domain).range([0, availableWidth]);
			...
			// in blur
			filter.append("feGaussianBlur").attr("in", "Source") //"SourceAlpha")
			.attr("stdDeviation", 2).attr("result", "blur");
			...
			var l = new layout().data(data).extent(domain).size([availableWidth, availableHeight]).fit();

			var zoomLayer = g.select("g.nv-zoom").selectAll("rect.nv-zoom").data([_d]);
			zoomLayer.enter().append("rect").attr({
				"class": "nv-zoom",
				x: 0,
				y: 0,
			...
			}).style("fill-opacity", 0).call(zoom);

			zoomLayer.exit().remove();

			var x = d3.time.scale().domain(domain).range([0, availableWidth]);

			var startScale = d3.time.scale().domain(domain).range([0, availableWidth]);

			axis.scale(x).orient("bottom").tickSize(-availableHeight, 0).tickPadding(7).tickFormat(d3.locale(localeDef).timeFormat.multi([[".%L", function (d) {
			...
			var xAxisG = g.select(".nv-x.nv-axis").attr("transform", "translate(" + margin.left + "," + (availableHeight + margin.top) + ")");

			if (showTooltip) {
				tooltip.chart(chart).wrapper(g.select(".fo-tooltipWrap")).x(margin.left).y(margin.top).width(availableWidth * 0.25).height(availableHeight).content(tooltipContent);
			}
			...
			var seriesG = g.select(".series").attr("transform", function (d) {
			...
			seriesContainer.enter().append("g").attr("class", "serie");

			seriesContainer.exit().remove();

			var lanes = g.select("g.series").selectAll("line.lane").data(l.lanes);

			lanes.enter().append("line").attr("class", "lane").attr("x1", 0).attr("y1", function (d) {
				return d.y;
			}).attr("x2", availableWidth).attr("y2", function (d) {
				return d.y;
			}).style("stroke", function (d) {
				return color(d, d.serie);
			}).style("stroke-width", 0.25);

			lanes.exit().remove();

			var items = g.select("g.series").selectAll(".serie").selectAll("rect.item").data(function (d) {
				return d;
			});

			items.enter().append("rect").attr("class", "item").attr("x", function (d) {
				return 0;
			...
			.style("fill", function (d, i) {
				...
				if (!showTooltip) {
					nv.tooltip.show([d3.event.pageX, d3.event.pageY], "<div style=\"width:" + Math.round(availableWidth / 4) + "px; white-space: normal;margin:5px;\">" + tooltipContent(d) + "</div>", "n", null, null, "xy-tooltip with-3d-shadow with-transitions");
				}
				...
				if (!showTooltip) {
					...
					var index = l.line.map(function (item) {
						return item.context;
					}).indexOf(d.context);
					if (index >= 0) {
						currentIndex = index;
			...
			chart.redraw = function () {
				lanes.transition().attr("x1", 0).attr("y1", function (d) {
					return d.y;
				})
				...
				.attr("y2", function (d) {
					...
					return color(d, d.serie);
				}).style("stroke-width", 0.25).attr("x2", availableWidth * zoom.scale());
				...
				.attr("x", function (d) {
					...
					return d.type == "instant" ? d.y + d.height / 2 - 2.5 : d.type == "process" ? d.y + d.height / 2 - 3.5 : d.y;
				}).attr("width", function (d) {
					return d.end * zoom.scale() - d.start * zoom.scale() < 5 ? 5 : d.end * zoom.scale() - d.start * zoom.scale();
				}).attr("height", function (d) {
					return d.type == "flow" ? d.dy : d.type == "instant" ? 5 : 7 // || d.type == "process") ? 5 : d.height
					;
				}).attr("rx", function (d) {
					...
					return d.type == "flow" ? _currentEvent && d.context == _currentEvent.context ? 0.5 : 0.05 : _currentEvent && d.context == _currentEvent.context ? 0.7 : 0.3;
				})
				...
				// })
				.style("stroke-width", function (d) {
					return d.type == "flow" ? _currentEvent && d.context == _currentEvent.context ? 2 : 0.3 : _currentEvent && d.context == _currentEvent.context ? 2 : 1.5
					...
					return d.context == _currentEvent.context ? d3.rgb(color(d, d.serieIndex)).darker(0.7).toString() : color(d, d.serieIndex);
			...
			var navigate = function navigate() {
				...
				if (showTooltip) tooltip.anchor(currentEvent.type == "flow" ? currentEvent.y + currentEvent.dy / 2 : currentEvent.y + currentEvent.height / 2).color(color(currentEvent, currentEvent.serieIndex)).show(currentEvent);
			...
			var translateWrap = function translateWrap(e) {
				chart.redraw();
				g.select("g.nv-x.nv-axis").transition().duration(500).ease("cubic-in").attr("transform", "translate(" + (-e.start * zoom.scale() + margin.left + 2 + availableWidth * 0.25) + "," + (availableHeight + margin.top) + ")");
				g.select(".series").transition().duration(500).ease("cubic-in").attr("transform", "translate(" + (-e.start * zoom.scale() + margin.left + 2 + availableWidth * 0.25) + "," + margin.top + ")");
			};
			...
			var onZoomWithTooltip = function onZoomWithTooltip() {

				var e = current();
				if (prevZoom && prevZoom != zoom.scale()) {
			...
			var onZoomWithoutTooltip = function onZoomWithoutTooltip() {
				chart.redraw();
			};
			...
			if (showTooltip) {
				...
				zoom.on("zoomend", onZoomWithoutTooltip).on("zoom", function () {
					g.select("g.nv-x.nv-axis").transition()
					// .duration(500)
					// .ease("cubic-in")
					.attr("transform", "translate(" + zoom.translate()[0] + "," + (availableHeight + margin.top) + ")");
					g.select(".series").transition()
					// .duration(500)
					// .ease("cubic-in")
			...
			if (_currentEvent) {
				...
				var index = l.line.map(function (d) {
					return d.context;
				}).indexOf(_currentEvent.context);
				if (index >= 0) {
					currentIndex = index;
					if (showTooltip) {
...
//# sourceMappingURL=../v2.nvd3-widget/nv.d3.ext.timeline.js.map