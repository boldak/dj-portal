...
define(["angular", "angular-oclazyload", "widgets/data-util/adapter"], function (angular) {

  var m = angular.module("app.widgets.data-widget", ["oc.lazyLoad", "app.widgets.data-util.adapter"]);

  m.factory("DataWidget", ["$ocLazyLoad", "APIProvider", "APIUser", "adapter", "pageSubscriptions", function ($ocLazyLoad, APIProvider, APIUser, adapter, pageSubscriptions) {

    $ocLazyLoad.load({
      files: ["/widgets/table/data-widget.css"]
    });

    var DataWidget = function DataWidget($scope, params) {
      ...
      $scope.removeSubscriptions = function () {
        ...
        for (var i in subscriptions) {
          if (subscriptions[i].emitter == $scope.widget.instanceName || subscriptions[i].receiver == $scope.widget.instanceName) subscriptions.splice(i, 1);
        }
      };
      ...
      $scope.RGBA = function (hex, opacity) {
        if (angular.isUndefined(hex)) return "rgba(" + 256 + "," + 256 + "," + 256 + "," + 1 + ")";
      ...
      $scope.getCellBackground = function (row, col, value) {
        ...
        return $scope.RGBA($scope.widget.decoration.color[index], $scope.widget.decoration.opacity ? $scope.widget.decoration.opacity : 0.5);
      };

      $scope.getCellStyle = function (row, col, value) {
        //console.log(arguments);
        if (arguments.length < 3) return { background: "transparent" };
...
//# sourceMappingURL=../v2.table/data-widget.js.map